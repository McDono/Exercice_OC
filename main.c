...
int main(int argc, char *argv[])
{
	...
	while (continuer)
	{
		...
		case SDL_KEYUP:
			...
			{
			case SDLK_SPACE:
				space_enfonce = 0; //Space n'est plus appuy�
				break;
			}
			break;
		case SDL_KEYDOWN:
			...
			case SDLK_ESCAPE:
				...
				break;
			case SDLK_SPACE:
				if (!space_enfonce) //Si space n'est pas d�j� appuy�
				{
					space_enfonce = 1; //On signal que maintenant space est appuy�

					if (pause) //Si le jeu est en pause, on met fin � la pause
					{
						pause = 0;
					}
		...
		if (!pause) //Si le jeu n'est pas en pause
		{
			...
			if (tempsActuel - tempsPrecedent >= 100) //Si au mmoins 100ms se sont �coul� depuis la derniere modif de temps
			{
